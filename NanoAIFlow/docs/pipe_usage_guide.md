...
> 阅读建议：先理解 `Pipe` 的签名契约，再理解 pipeline 的调度/顺序策略，最后再调优线程与背压参数。
...
实践建议：执行策略没有绝对优劣，核心是让“耗时特征相近、资源争用模式相近”的阶段采用相近策略，避免把所有问题都推给共享池，或把所有阶段都拆成昂贵的专用池。
...
这一能力的边界非常重要：框架提供的是“协作式”取消，而非强杀线程。也就是说，stage 是否能快速停下，取决于业务逻辑是否主动检查 token 并在合适位置退出。
